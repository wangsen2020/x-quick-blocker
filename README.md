...
- **一键屏蔽** —— 每条推文的操作栏最右边多一个禁止符图标，点一下直接拉黑作者，省掉「⋯ → 屏蔽 → 确认」三步。图标默认与 X 其它操作图标同色，悬停变红并显示提示
...
打开 x.com，每条推文的操作栏最右边会多一个禁止符图标（🚫 形状），点一下即可。

图标不带文字，靠 tooltip 说明用途——悬停时显示「屏蔽 @用户名」，跟随浏览器语言自动切换（内置英文、简体中文、繁体中文、日文，见 `_locales/`）。
...
src/panel.css             样式
_locales/                 按钮文案的多语言（en / zh_CN / zh_TW / ja）
docs/                     README 截图
```
